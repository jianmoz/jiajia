        ...
        methods:{
            ...
            countDown(){
                ...
                var timer = setInterval(()=>{
                    count--;
                    this.verify_info = '剩余'+ count + '秒';
                    if(count === 0){
                        this.disabled = false;
                        count = 60;
                        this.verify_info = '发送验证码';
                        clearInterval(timer);
                    }
                }, 1000);
            }
...
<style scoped>
    ...
    .password_body .password_email button{
        position: absolute; right: 10px; top: 10px; height: 30px;
        border-radius: 3px; border:none; padding: 5px;
    }
    .password_body .password_btn{ height:50px; margin:10px;}
    .password_body .password_btn button{
        width:100%; height:100%; background:#e54847; border-radius: 3px; border:none;
